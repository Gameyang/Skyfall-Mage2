...
무기 상점은 단순한 스킬 해금 메뉴가 아니다. 플레이어가 `지금 가진 슬롯 3개를 어떤 공격 사이클로 구성할지` 결정하는 정비 구간이다.
...
- 재고는 상점에 입장한 순간 고정된다. 나갔다가 돌아와도 같은 상점 세션에서는 바뀌지 않는다.
...
6. 상점 구매 시 `WeaponInstance`를 인벤토리에 추가하고 coin을 차감한다.
7. 상점 UI에 구매, 비교, 즉시 장착, 슬롯 교환을 붙인다.
8. 등급/가격/스탯 예산을 wave 진행도와 함께 밸런싱한다.
