...
fn fragmentMain(in: VertexOut) -> @location(0) vec4f {
  ...
  let saturation = (maxChannel - minChannel) / max(0.001, maxChannel);
  let grayBlockMask = select(1.0, 0.0, saturation < 0.16 && maxChannel < 0.78);
  let alpha = color.a * clamp(params.atlas.z, 0.0, 1.0) * grayBlockMask;

  if (alpha <= 0.01) {
    discard;
  }

  let glow = 1.04 + smoothstep(0.12, 1.0, color.a) * 0.42;
